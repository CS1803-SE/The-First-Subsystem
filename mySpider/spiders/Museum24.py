...
class Museum24(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        item["address"] = response.xpath('/html/body/div[1]/div/footer/div/div[2]/div/div[3]').xpath("string(.)").extract_first()
        item["openingTime"] = StrFilter.filter(response.xpath("/html/body/div[1]/div/section[5]/div/div/div[2]/div/div[2]/div[2]").xpath("string(.)").extract_first())
